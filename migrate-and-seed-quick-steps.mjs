 ...
 * 1. Adds quick_steps column to sop_library if it doesn't exist
 * 2. Populates quick_steps for all SOPs
 * Run: node migrate-and-seed-quick-steps.mjs
 */
...
try {
  await connection.execute("ALTER TABLE sop_library ADD COLUMN quick_steps TEXT NULL");
  console.log("✅  Column quick_steps added to sop_library");
} catch (err) {
  if (err.code === "ER_DUP_FIELDNAME") {
    console.log("ℹ️   Column quick_steps already exists — skipping ALTER");
  } else {
