...
export const TODO: TodoItem[] = [
  ...
  },
  {
    ...
    description:
      "COMPLETE (tRPC surface) — Feb 2026 full audit of all 53 procedures. Every one of the 17 `protectedProcedure` handlers correctly resolves `dbUser = getUserByOpenId(ctx.user.openId)` and scopes queries to `dbUser.id + dbUser.wineryId`; every one of the 29 `ownerProcedure` handlers throws before reading data. The feared 'seed-owner-001 fallback on missing session' does not exist in current code — protectedProcedure blocks anonymous callers at the tRPC gate itself. All 7 publicProcedure endpoints reviewed and confirmed genuinely safe for anonymous access (signup mutations, aggregate counts, or Rich's intentionally-public /stats trust-signal). REMAINING: the raw `/api/admin/*` Express handlers (see next todo item) still sit outside the gate wall.",
    priority: "p0",
    effort: "~4-6 hours",
    status: "done",
    category: "Safety",
  ...
  {
    id: "api-admin-express-handler-guard",
    title: "Gate the raw `/api/admin/*` Express handlers with an owner-cookie check",
    description:
      "The tRPC surface is audit-clean, but the raw Express handlers under /api/admin/* (currently /api/admin/qr-scans + /api/admin/health-status) sit OUTSIDE the gate wall because /api/ requests skip the HTML wall by design. Both return low-sensitivity aggregate data today (scan counts, health probes), but the pattern should be tightened before any endpoint returning member data joins the group. Fix: add a small `requireOwnerCookie` express middleware that checks the ow_gate cookie (owner tier) and 401s otherwise, then apply it to every /api/admin/* handler.",
    priority: "p1",
    effort: "~45 min",
    status: "not-started",
    category: "Safety",
  ...
  {
    id: "confirm-mrr-public-exposure",
    title: "Confirm MRR is safe to expose on `campaignMetrics.getLatest`",
    description:
      "The public `campaignMetrics.getLatest` procedure (used on /pricing for the founding-member counter) also returns the `mrr` field (AUD cents). Anyone can call this endpoint anonymously and see current Monthly Recurring Revenue. Rich published MRR as social proof historically. Confirm this is intentional; if not, either strip `mrr` from the public shape or move that field to `getHistory` (already ownerProcedure).",
    priority: "p1",
    effort: "~5 min (Rich confirms) + ~10 min code if stripping",
    status: "not-started",
    category: "Safety",
    updatedAt: "2026-02-15",
  },
