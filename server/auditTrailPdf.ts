...
export async function generateAuditTrailPdf(req: Request, res: Response): Promise<void> {
  try {
    ...
    // Header
    doc.fontSize(20).fillColor("#000").text("Ownology Compliance Audit Trail", { align: "left" });
    doc.moveDown(0.2);
    doc.fontSize(10).fillColor("#555");
    doc.text(`Winery: ${owner.name ?? "—"}`);
    ...
    doc.fontSize(8).fillColor("#888").text(
      `Ownology — Cellar Intelligence Platform for Winemakers. ` +
      `Generated by user ${owner.openId}. Page-by-page audit trail of compliance-relevant cellar activity.`,
      { align: "center" }
    );
