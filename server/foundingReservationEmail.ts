...
function renderOwnerAlertHtml(p: ReservationPayload): string {
  ...
  <h2 style="font-family: Georgia, serif; margin: 0 0 12px; color: #78350f;">🍇 Founding-Member reservation — Slot #${p.slotNumber}</h2>
...
export async function sendReservationEmails(payload: ReservationPayload): Promise<{ customerSent: boolean; ownerSent: boolean }> {
  ...
  try {
    const send = await resend.emails.send({
      ...
      to: [payload.email],
      subject: `Slot #${payload.slotNumber} reserved — Ownology Founding Member`,
