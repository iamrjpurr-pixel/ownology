...
**RED keyword removed from customer-facing copy.** The "Reply RED to lock my onboarding →" button on `/hi/<slug>` pages had zero context for prospects — "RED" was a proposed inbound-SMS filter keyword that was never actually wired to any server-side automation (the reply classifier uses Claude sentiment analysis, not keywords). Button relabelled to "Text me to lock my onboarding →" in `HiContact.tsx`. SMS body prefix `"RED — Hi, it's Adrian..."` rewritten to `"Hi Rich, it's Adrian from..."` in `buildSmsReplyHref` + `buildWaHref` — reads like the customer composed it themselves. Internal admin labels ("Reply RED" A/B card in `AdminContacts`, `AdminDev` blurb, `AdminOperatorGuide` copy) all swapped to "Text reply" for consistency.

**PWA install nudge — 7-day snooze, hero variant on `/dashboard`.** `PwaInstallBanner` previously stored `ownology_pwa_banner_dismissed="1"` as a permanent flag — dismiss once, never see again. Now stores a snooze-until timestamp (default 7 days). Legacy `"1"` values are cleared on next visit so old dismissers get re-nudged. Added a HERO variant that fires on `/dashboard` (the post-login landing): fatter padding, thicker amber border, Fraunces headline "Add Ownology to your home screen — 10 sec setup", value-forward subtitle ("Faster than the browser · Opens like an app · Stays signed in · No app store needed"). Regular subtle strip still used everywhere else. Suppressed on prospect surfaces (/try, /hi/, /audit/, /join, /founding-member/success) so it never intrudes on conversion moments.

**Admin: `/admin/users` surface shipped.** New `AdminUsers.tsx` page + `adminUsersRouter` (list, recentLoginTokens, sendFreshMagicLink, stats). KPI strip (total · admins · last-30d), search box (email · name · winery), 200-row table with per-row expandable panel showing the last 10 magic-link tokens (pending / consumed / expired). "Send fresh link" button per row calls a new owner-only path that bypasses the public 3/hr rate-limit — for unblocking stuck sign-ins. Route wired at `/admin/users`. Deliberately no role-toggle (managed via `ADMIN_EMAILS` env — single source of truth) and no user-delete (data preservation). Also refactored magic-link email dispatch into a shared `issueMagicLink()` helper so public POST + admin resend share the exact same template + Resend send path.
...
**Instagram handle backfill (Perplexity Sonar).** New tRPC procedure `outreach.backfillInstagramHandles({ limit, dryRun })` — owner-scoped, calls Perplexity Sonar for up to N winery contacts whose `notes` field has no `IG:` / `Instagram:` / `instagram.com/…` marker yet. Prompt asks for strict JSON `{business, personal, confidence}` with explicit "return null if not verified, do not guess" instructions. Validates handle shape against Instagram's rules (2-30 chars, lowercase letters/numbers/underscores/periods) before writing. Appends `[auto-IG YYYY-MM-DD] IG: <business> · IG-personal: @<personal>` to notes so it's auditable and the client-side extractor in `AdminOutboundQueue.tsx` picks it up automatically — a "DM on Instagram" primary CTA appears on the next queue reload without any migration or backfill on the queue itself. Cost ~$0.004/query, 200ms sleep between calls to stay under Perplexity's soft rate ceiling, `dryRun: true` returns the candidate list without spending. Live-verified: found `IG: seppeltwines` for Adam Carnaby @ Seppelt Wines in a 3-contact run (2 correctly returned null instead of guessing).
...
**P1 — Clean/Sanitise quick-entry tiles.** New tRPC procedure `cellarTasks.logCompleted` — single-write insert that creates a Clean or Sanitise `cellar_tasks` row already stamped `completedAt` + `completedBy`. Sidesteps the flaky drizzle MySQL insertId return that made the naive `add → then complete` two-step lose the completion write (verified in the field: id=21 sat with completedAt=null before the fix; id=23 landed with completedAt populated after). Client change: `/quick-entry` now renders a "Cellar hygiene · one tap" strip above the 7 event tiles with 🧽 Clean + 🧴 Sanitise buttons. Tapping opens a modal with an equipment name input (datalist-suggested from `getUsedTanks` + wine batches) and an optional notes textarea. Log button fires the mutation, invalidates the cellar-tasks list, and toasts "Sanitised · Tank 4 logged". Feeds the /admin/cellar-board RAG status — a completed sanitise resets the equipment sanitation-freshness window and flips it green.

**P1 — `/vineyard` tight rebuild.** Trimmed OBSERVATION_TYPES from 12 → 6: kept `budburst / flowering / veraison / harvest_date / yield_estimate / other`, removed spray/irrigation/canopy management/disease scouting/pest scouting/disease-pest event. Historical rows using the removed enum values still render (they just don't appear in the picker). Header subtitle changed from "Block register · Phenology · Observations" → "Block register · Phenology · Yield". Empty-state copy tightened. Added an honest scope footer at the bottom of the block list: "Vineyard here is the block register + phenology + yield only. Spray diaries, canopy notes, and pest scouting live with your viticulturist's tool of choice — Ownology's job starts when the fruit hits the tanks."

**P2 — Cross-page copy consistency audit.** Fixed the Terms page (`/terms`) which listed wrong subscription prices ($19/$49/$99 monthly, $190/$490/$990 annual) — actual tiers from `data/pricing.ts` are Cellar Hand $22 founding / $28 retail · The Press $44 founding / $59 retail · The Vigneron $88 founding / $124 retail. Rewrote the Payment section to use the correct numbers, added the "annual = 10× monthly" ANNUAL_MULTIPLIER note, and dropped the false "The Vigneron includes three seats" line (the actual feature is "Team seats (rolling out with multi-tenant)"). Unified terminology: "Founding Member" → "Founding Cohort · 2026" across Terms.tsx, Refund.tsx, and the Pricing.tsx FAQ so all four surfaces (Terms/Privacy/Refund/Pricing) speak the same language.
