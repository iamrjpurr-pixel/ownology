...
- [x] **The Coherence Pass — publish 225 ghost questions into public /cellar-journal + site-wide cross-linking** ✅ (Feb 2026, this session). One coherent value-engineering sweep instead of scattered small edits: (a) `scripts/migrate-ghost-questions-to-journal.mjs` publishes all 225 ghost questions as full journal entries with derived slugs, WBS-resolved citations (SOPs + Wine Bible chapters, D-prefixed white-bible fallback), teaser/full/diagnosis split, category→topic mapping, wine_type→red/white/both mapping, `source='ghost.seed'`, idempotent re-run. Journal now serves **253 published entries** (28 real from tutor.ask/freeRun + 225 ghost.seed) — sitemap.xml auto-grew from ~30 to **254 URLs**, RSS feed picks them up automatically. (b) Added `journal_slug VARCHAR(200)` column to `ghost_questions` + index (`scripts/add-ghost-journal-slug-column.mjs`) so Cellar Brief cards can deep-link. Engine + WbsCache load it; every ghost pick now carries its journal slug. (c) Cellar Brief `GhostQuestionBlock` renders **"Read full answer + citations →"** deep-link (`/cellar-journal/<slug>?from=cellar-brief`) at bottom of expanded Q+A — feeds logged-in users into the SEO surface. (d) Homepage hero got a subtle **"📚 Or browse 250+ winemaking Q&As in the Cellar Journal →"** link below the primary CTAs (`?from=homepage-hero`). (e) Homepage footer completely reorganised from a 10-link horizontal scroll into a proper 4-column grid: Ownology brand + **Product** (Pricing, Cellar Brief, Ask Ownology, Sample vintage log, Merch) + **Library** (Cellar Journal, Knowledge, Regulations, Compliance, Blog) + **For** (Home winemakers, InnoVint users, Vintrace users, Why Ownology) + copyright row with RSS + Sitemap links. Killed the dummy `href="#"` Privacy/Terms/Contact anchors. (f) `/hi/:slug` SMS cold-prospect landing gained a **"✦ Recently asked in the journal"** dashed-border social-proof card above the CTAs — deterministic-per-contact so the same visitor sees the same question across refreshes, with `?from=hi-<slug>` attribution. Verified by testing agent (iter 18: **21/21 backend pytest PASS**, zero frontend UI/design/integration bugs; two minor notes were spec-wording nits, not implementation defects).
