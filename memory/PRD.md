...
- **New tRPC router `industryNews`:** `refresh` · `list` · `itemContacts` · `generateOpener` · `archive`. `generateOpener` extends existing `claudeRewriteOne` with an optional `industrySignal` (headline + dek + source + sourceUrl + publishedAtIso + region) that Claude is instructed to lead with. Result saves to `smsDraftOverride` + stamps `hookSourceUrl` + `hookTier="industry_signal"`.
- **Site-wide banned-copy sweep:** removed every "cellar AI / second brain / AI apprentice / cellar apprentice / cellar-intelligence AI" mention from user-facing surfaces across 15 files: Home hero H1, `/why-ownology`, `/vs/innovint-vintrace` (5 spots), `/how-we-trace`, `/knowledge`, `/guide`, `/hi/:slug` personal landing tiles + QMS variants, `UserJourneyDeck`, outreach kit (LinkedIn DM x3, mailto templates x5, IG DM, weekly digest signoff, Claude system prompt), `ownology-descriptor.ts` category noun + sell stack. Unified positioning: **"Quality and risk management for winemakers — across the whole business. Productivity and profit compound year on year."** Owen persona kept; role label swapped from "AI apprentice" → "working memory".

**Jul 2026 session — PWA session-aware launch + version banner + cache reset + desktop-safe SMS + region editor — SHIPPED (earlier):**
...
- **Feature flag** (`client/src/config/ui.ts`): `useUiPillarsV1()` — defaults to `true` (new hero live), URL override `?ui=v0` renders original apprentice hero for A/B comparison during prospect calls. `?ui=v1` locks the new variant. Override stored in `sessionStorage` so a prospect who loads `?ui=v0` on a shared link keeps the classic hero for their session. Deletion strategy documented in the flag file: kill the flag + old hero JSX once Rich signs off after ~4 weeks of contact-list exhaust.
- **`Home.tsx::Hero()`**: unchanged original hero JSX preserved verbatim, wrapped in `{usePillarsV1 ? <HeroPillarsSection/> : <old hero>}`. Zero-cost revert.
- **Work Mode nav pillar tags** (`components/WorkModeLayout.tsx`): each of the 5 bottom-nav items now carries a `pillar` field (`Ask→Learn`, `Press→Know`, `Log→Do`, `Tasks→Do`, `More→Guide`) rendered as a tiny italic serif label under the tab icon+label. Additive — nav routing/hit-targets unchanged; flip 5 spans off to revert.
- **`/site-map` link removed from `SiteFooter`** — reduces prospect exposure to internal admin/dev URLs. `/site-map` route + `Sitemap.xml` still live for Google + team use.
- **Value-engineering strategy**: single flag file, additive DOM changes, no destructive edits. Kill-switch is one boolean; full revert is `?ui=v0` per-session or `useUiPillarsV1 → false` globally. Old hero code stays warm for 4 weeks. Estimated total burn if we pivot: ~3k tokens (flag flip + component delete).
- **Verified end-to-end**: default `/home` renders 4-pillar hero (all 4 cards testable via `hero-pillar-{do,know,learn,guide}` testids); `/home?ui=v0` restores classic hero; `/free-run` (Work Mode) shows all 5 pillar labels; pytest smoke 14/14 green.


**`/ask` public SEO flywheel + Owen persona finalise — SHIPPED (Feb 2026, this session)**
- **`/ask` page (`client/src/pages/Ask.tsx`)**: public, no-signup, no-gate. Any visitor asks a winemaking question → Owen (via `trpc.tutor.ask` in `home_winemaker` mode) answers from the bible-RAG corpus (Red & White Wine Bibles + MoreWine! manuals). Answers grounded, cited, and every response surfaces the permanent `/cellar-journal/:slug` permalink so visitors + Google can bookmark and share.
- **Server changes (`server/routers/tutor.ts`)**: DIY path now `await`s `persistJournalEntry` (previously fire-and-forget) so we can return `journalSlug` + `journalIsNew` to the client. Added anonymous per-IP rate limiter (`5 asks / hour / IP`) — signed-in users are exempt. Rate-limit exceeded → 429 with friendly "Reserve a Founding Member seat" copy.
- **SEO wiring**: `/ask` added to sitemap (`priority=0.9, changefreq=weekly`), OG meta card injected server-side (`server/index.ts::ROUTE_META`), + `Helmet` client tags. Trinity dedupe clustering (`SIMILARITY_THRESHOLD=0.80`, `text-embedding-3-small`) already in `cellarJournalRouter.ts` — verified working: same question asked 3× returns same canonical slug + increments `askedCount`.
- **Gate wall**: `/ask` appended to `PUBLIC_EXACT` in both `server/index.ts` and `viteGateWall.ts` — verified public (`HTTP 200`) while `/admin` still 302 → `/try`.
- **Owen persona finalised in `/try` sandbox (`client/src/pages/Try.tsx`)**: replaced all "Rich and Gel" customer-facing "sends emails / daily tasks" copy with **Owen** (the AI cellar-hand). All "5:30am" references shifted to "7am" to match the daily Cellar Brief cron. "Rich & Gel" retained only in the "Our story" section (they're still the winery founders) + "Rich replies personally" in the 4-hour post-signup timeline (a human, not the AI). "Gel says:" callout renamed to "Owen says:".
- **Owen hover-tooltip introduction**: dotted-amber-underlined `<abbr>` on the first "Owen" mention in Step 1 with title text: *"Owen (also Eoghan / Owain) — a Celtic name meaning 'well-born' or 'noble'. Owen is Ownology's AI cellar-hand: he reads yesterday's logs while you sleep and writes the brief you wake up to."*
- **Verified end-to-end**: `curl` to `/api/trpc/tutor.ask` returns answer + citations + `journalSlug`; browser flow (click sample → answer visible → permalink `/cellar-journal/racking-lees-when-should-i-rack-off-the-gross-lees` links to working journal entry with `askedCount=3`); sitemap contains `/ask`; 14/14 pytest smoke green.
...
- [x] **Phase 2 NOT NULL + FK lockdown** ✅ (Feb 2026, this session). All 9 customer-domain tables that had user-linked backfill paths are now `winery_id INT NOT NULL` with a foreign key to `wineries(id) ON DELETE CASCADE`. Bootstrap migration auto-applies on every boot (idempotent: sweeps orphan rows, checks for NULLs, only flips when safe, only adds FK if missing via INFORMATION_SCHEMA). Verified: `fk_vle_winery`, `fk_wb_winery`, `fk_ce_winery`, `fk_ct_winery`, `fk_barrel_winery`, `fk_pkg_winery`, `fk_vb_winery`, `fk_vo_winery`, `fk_tr_winery` all live in the railway DB. Database-layer cross-winery isolation is now belt-and-suspenders.
...
- [x] **Cellar Brief MVP** ✅ (Feb 2026, this session). Backend engine (`server/cellarBriefEngine.ts`) + tRPC router (`server/routers/cellarBrief.ts`) shipped in prior session. THIS session: built the mobile-first `/cellar-brief` page (`client/src/pages/CellarBrief.tsx`), wired the route into `App.tsx` (eager — daily-driver killer feature), and shipped `scripts/seed-twelve-day-vintage.mjs` (Adelaide Hills region, 4-tank scenario exercising every stage: healthy Shiraz primary, cool-climate Chardonnay primary, STUCK Shiraz ferment with temp spike → ATTENTION + decision-due, Pinot Noir post-press MLF active). Also fixed: (1) White Wine Bible chapter refs (taxonomy mismatch — white chunks were ingested with `D`-prefixed WBS codes; `resolveGrounding` now probes both bare + `D`-prefixed for whites), (2) broken `history` pagination cursor (was ignoring `beforeGeneratedAt`), (3) gated `history.useQuery` on `enabled: showHistory` to avoid wasted round-trip, (4) suppressed ThemeOnboarding + ThemeSuggestion on `/cellar-brief` so bottom action surface stays clickable. Verified via testing agent (iter 16: 8/8 backend pytest + 12/13 frontend Playwright pass; the one fail was the white-bible bug now fixed). Engine handles 13 active vessels at winery_id=1 in <300ms; 2-sentence LLM exec summary writes correctly via Emergent Universal Key.
- [x] **Cellar Brief shortcuts on cold-prospect surfaces** ✅ (Feb 2026, this session). Added prominent amber-bordered "✦ See a live Cellar Brief" CTA to the homepage hero between Free Trial + How It Works (`?from=homepage-hero` for attribution), and a tertiary dashed-border CTA on `/hi/:slug` SMS landing below the existing sample-vintage-log link (`?from=sms-<slug>`). Lets organic prospects + VIVID SMS leads see the killer feature within 3 seconds of opening, while the `/admin/funnel` page picks up the new source rows automatically for A/B insight (sample-log mockup vs live Cellar Brief — which converts better?).
- [x] **Ghost Questions seed + Cellar Brief integration** ✅ (Feb 2026, this session). Added `answer TEXT` column to `ghost_questions` (`scripts/add-ghost-questions-answer-column.mjs`). Built `scripts/seed-ghost-questions.mjs` — generates 5 Q&As per (WBS code × wine_type) combo via gpt-5.4-mini grounded in `diy_knowledge_chunks` + `sop_library`; final coverage **225 questions** across 15 WBS codes × 3 wine_types (75 red / 75 white / 75 general). Each Q&A: 80–140 char question, 80–140 word answer with concrete numbers, plus category tag (fermentation/so2/mlf/etc.) and difficulty (beginner/intermediate/advanced). Engine extended with `pickGhostQuestion()` — vessel-stable deterministic hash pick from pool of WBS×wine_type-matching questions, with `general` fallback when no exact wine_type match exists. WbsCache eagerly loads all active ghost questions into `ghostsByKey` map (no N+1). Frontend `GhostQuestionBlock` renders a collapsible "✦ Worth knowing" Q+A block in each expanded card, below the grounding chips and above the action buttons. Verified by testing agent (iter 17: 13/13 backend pytest + frontend Playwright PASS, zero critical/minor frontend issues; the only minor reported was a 3.1/red coverage gap which has since been backfilled to perfect 75/75/75 balance).
- [x] **Sparkling wine capability — end-to-end** ✅ (Feb 2026, this session). Audit found zero procedural sparkling content anywhere (bibles mentioned it in-passing only; no SOPs, no ghost questions, no journal entries, no WBS branch). Shipped a first-class sparkling capability without needing a dedicated bible. Changes: (1) `WineColor` type extended with `"sparkling"`; `SPARKLING_HINTS` (sparkling/cuvée/méthode/pét-nat/prosecco/blanc de blancs/brut/etc.) + `SPARKLING_VARIETIES` (Meunier, Glera) drive intent detection — sparkling wins over the underlying grape color, so "Sparkling Chardonnay Cuvée" routes to the 9.x branch while still Chardonnay stays "white". (2) New `9.1–9.9` WBS branch covering Base Wine, Cuvée Blending, Tirage, Prise de Mousse, Aging on Lees, Riddling, Disgorging, Dosage, Bottling Under Pressure (Charmat). `STAGE_TO_WBS` now has a `sparkling` sub-branch per stage. (3) `resolveGrounding` filters red-flow SOPs (title-match on "Red Wine", "Pump-Over", "Cap Manage", "Punch", "Skin Contact") when card is white/sparkling; prefers White Wine Bible for sparkling (sparkling base is white-wine flow); probes `D`-prefixed WBS codes for sparkling same as whites. (4) `scripts/seed-sparkling-ghost-questions.mjs` LLM-generates **54 sparkling Q&As** via gpt-5.4-mini (6 per WBS code × 9 codes), Tasmania-forward answers referencing House of Arras, Jansz, Clover Hill, Pipers Brook, Delatite; plus Yarra Valley (Chandon), King Valley (Prosecco / Dal Zotto), Adelaide Hills (Deviation Road, Petaluma), Macedon Ranges (Hanging Rock), Great Southern WA. Concrete numbers baked in (tirage 24 g/L sugar, 5–6 bar prise de mousse, dosage classes from brut nature 0–3 g/L to doux >50 g/L, aging on lees 12/24/36+ months). (5) `cellar_journal.wine_type` ENUM extended with `'sparkling'`; migration script re-runs and publishes the 54 into `/cellar-journal` under topic tag "Sparkling & Bubbles". Journal grew from 253 → **307 entries**; sitemap from 254 → **308 URLs**. (6) Demo seed adds a Tank 20 Sparkling Chardonnay Cuvée scenario (Adelaide Hills, 12-day base-wine ferment, low pH 3.05 / TA 9.4) so the /cellar-brief page shows a live sparkling card. Verified by testing agent (iter 19: **32/32 backend pytest PASS + zero frontend UI/design/integration issues**).
- [x] **Real MoreWine content ingestion + full corpus regeneration** ✅ (Feb 2026, this session). Audit revealed `morew_white_outline` was LLM-synthesised (mislabelled) — dropped 7 chunks. Fetched 15 real MoreWine PDFs from their CDN — 2 sparkling protocols (Prise de Mousse Yeast + ProElif) + 13 specialist MoreManuals (SO₂ Mgmt/Protocols, Yeast Pairing, Yeast Rehydration, Oxygenation, Malolactic Bacteria, Inert Gas, Sanitation, Oak Info, Oak Barrel Care, pH Meter, Bench Trials, Fining Agents) via `scripts/ingest-morewine-pdfs.py`. Ingested **106 new chunks** into `diy_knowledge_chunks` (Python + pypdf extraction, WBS-tagged with primary + 1-2 secondary codes for cross-topic surfacing). Total corpus now **283 published chunks across 47 source_docs**; MoreWine is 55% of the corpus (156 chunks). Then upgraded `resolveGrounding` iteration order for whites/sparkling (probe D-prefix codes FIRST so bibles don't get crowded out of the 2-chunk cap by specialist manuals); added `SOURCE_LABEL` map for readable chip text; extended sparkling `preferred` list to include the 2 sparkling protocols. Fixed the Ask Ownology tutor (`tutor.ts`) to: label all 15 new MoreWine sources with proper human names in the LLM context, extend the home-scale boost to include specialist manuals (+0.4) and sparkling protocols (+0.6), route sparkling sources into white+sparkling queries, and stop referencing the deleted `morew_white_outline`. Finally regenerated **all 279 ghost questions** (54 sparkling + 225 still-wine) with MoreWine grounding — sparkling seed now fetches `morew_sparkling_yeast` + `morew_sparkling_proelif` chunks before prompting; still-wine seed's existing `getContextForWbs` naturally picks up the new specialist manuals via keyword match. Answers now cite real MoreWine numbers verbatim (24 g/L tirage, 0.6 g/hL Phosphate Titres, 2–2.25" fill line, 5% starter, pH >2.9, FSO₂ <15 ppm, SO₂ 125–200 ppm ranges, 35-40°C yeast rehydration at 10× water). Republished all 279 into `/cellar-journal` — journal serves **307 total entries** (28 real user-asked + 279 ghost.seed) with fresh MoreWine-grounded content. Verified: Ask Ownology tutor now returns answers explicitly citing MoreWine ("MoreWine's standard approach is to rehydrate in clean water at about 35-40°C…") when asked about tirage / rehydration / SO₂. Cellar Brief grounding chips include Sparkling Wine Protocol on sparkling cards, SO₂ Management on ferment cards, White Wine Bible on white cards, Red Wine Bible on reds.
...
- [x] **Winery rename / region / brand colour** in `/admin/settings` ✅ (Feb 2026, this session). See "Completed" entry below.
- [x] **Trigger cascade from a homepage CTA** ✅ — already shipped (Auto-Cascade on Landing entry above, plus manual "✦ Preview harvest mode" button).
- [x] **Auto-bypass "Create account" modal in dev** ✅ — `freeRun.authCheck` already returns `isAuthenticated: true` when dev-bypass is active, because `ctx.user` is auto-injected with `DEV_BYPASS_USER`. Verified live.
...
- [x] **`admin.llmStats` → publicProcedure** ✅ (Feb 2026, this session). The `/stats` page now works for any visitor (including post-auth-enforced site) as a trust signal — winemakers can verify Ownology isn't burning their data into runaway LLM costs. Zero PII exposed (aggregates only). Sibling owner-only mutations `resetLlmStats` / `resetDailyBudget` / `resetFreeRunQuota` stay locked.
...
- [x] **Backlog sweep: 4 polish items (Feb 2026, this session)**.
  - **Conversion CTA on `/audit/<slug>`**: amber footer card "Want a live audit trail for your winery? → SEE PRICING" links to `/pricing?from=public-audit-<slug>`. Hooks into the existing `pricing.logView` pipeline — every regulator/visitor pageview is now an attributed conversion event surfacing on `/admin/funnel`. Brand-coloured background on the published winery's audit page.
  - **SOP markdown/PDF export brand colour — not applicable**: the SOP library export is platform-reference content (master library, pre-baked static PDF), not a per-winery artefact. Branding it per-winery would mean regenerating a large PDF on every request for every tenant — wrong tradeoff. The actually-customer-facing branded outputs (compliance audit PDF + public audit page) are already done.
  - **`seed-mock-data.mjs` fixed**: corrected `openId` → `open_id`, `createdAt` → `created_at`, `NOW()` → `Date.now()` for the bigint column. Added Phase 2 winery_id resolution: ensures a Default Winery exists, attaches the owner, then threads `WINERY_ID` into every customer-domain INSERT (wine_batches, vintage_log_entries, barrels, cellar_equipment, cellar_tasks, tank_reminders). Verified live: re-running the seed populated 36 new rows (4 batches + 8 barrels + 10 equipment + 10 tasks + 4 reminders), all with `winery_id=1`. FK constraints held.
  - **`seed-quick-steps.mjs` marked obsolete**: the script targets the Manus-era SOP id range (1-3, 15-17, 60001-60007) which was rebaselined during the migration to /app. Current sop_library uses ids 7-44 and is already fully populated with quick_steps by `scripts/seed-38-sops.mjs`. Added a coverage guard at the top of the script — exits cleanly when all SOPs already have quick_steps, with a pointer to the active successor script. Safe to keep around for historical reference; no longer footguns future developers with a flood of "No row found" warnings.
  - **PWA install banner — iOS Safari path added**: existing `PwaInstallBanner` only triggered on `beforeinstallprompt`, which iOS Safari **never fires** even though Ownology IS installable on iPhone via Share → Add to Home Screen. Added iOS UA detection in `usePwaInstall` (excludes in-app browsers FBAN/Instagram/CriOS/EdgiOS) and a Safari-specific banner state showing the share icon + "Tap Share button below, then 'Add to Home Screen'" copy. Verified live with spoofed iPhone UA: banner renders correctly at viewport bottom. iPhone winemakers now see the install path (~25% of cellar-floor traffic). Service worker intentionally deferred — adds meaningful cache-invalidation surface area for marginal gain; current banner works for >70% of installs without it.
...
- [x] **Per-winery logo on compliance PDF exports + winery rename / region / brand colour (Feb 2026)**.
  ...
  - Frontend `AdminSettings.tsx` gained a "Winery" section between Units and Theme suggestions: editable name + region inputs, brand-colour text input with live swatch preview, **logo URL input with live image preview that hides on 404**, plan badge, Save / Revert buttons gated on `dirty` state, FORBIDDEN flash for non-owner members. All inputs have `data-testid`s (`winery-name-input`, `winery-region-input`, `winery-brand-color-input`, `winery-logo-url-input`, `winery-save-btn`, `winery-revert-btn`).
  - UserMenu already pulls `admin.me.winery.name` → renames propagate to the top-right pill automatically.
  - **Branded audit PDF**: `server/auditTrailPdf.ts` now resolves the user's winery → fetches the logoUrl bytes (3s timeout, PNG/JPEG only, ≤2MB cap), embeds the logo top-left, prints the winery name as the H1 in the brand colour, draws a brand-coloured rule under the header, and surfaces the region in the metadata block. Footer attribution swapped to "{wineryName} — generated by Ownology". Falls back gracefully to text-only if the logo fetch fails or content-type is unsupported (pdfkit only supports PNG/JPEG; SVG/WebP skipped). Verified live: with `brandColor=#b45309` the new colour appears in both the title TJ stream and the rule stroke (0.7058 0.3254 0.0352 SCN). With `brandColor=null` falls back to black.
  - Test: `server/tests/test_winery_rename.mjs` — provisions owner + non-owner team member sharing the same winery, exercises all flows (read as owner, read as member, owner edit, non-owner FORBIDDEN, bad hex BAD_REQUEST, non-https logoUrl BAD_REQUEST, http logoUrl BAD_REQUEST, valid https logoUrl persists, clear region to null, empty-patch no-op). **19/19 assertions pass.** Cross-winery isolation test from Phase 2 still passes (regression confirmed).
  - Also flipped `admin.llmStats` → `publicProcedure` (zero PII; aggregates only) so `/stats` keeps working once auth is enforced site-wide. Sibling owner-only mutations (`resetLlmStats`, `resetDailyBudget`, `resetFreeRunQuota`) stay locked.
